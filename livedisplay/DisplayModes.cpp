...
static const char * kModePath = "/sys/devices/platform/soc/ae00000.qcom,mdss_mdp/main_display/display_mode";
static const char * kDefaultPath = "/data/system/default_display_mode";
...
Return<void> DisplayModes::getCurrentDisplayMode(getCurrentDisplayMode_cb resultCb) {
    std::map<int32_t, ModeInfo>::const_iterator current = kModeMap.begin();
    std::ifstream modeFile(kModePath);
    std::string value;
    modeFile >> value;
    if (!modeFile.fail()) {
        for (auto iter = kModeMap.begin(); iter != kModeMap.end(); ++iter) {
    ...
    DisplayMode mode { current->first, current->second.name };
    resultCb(mode);
    return Void();
}

Return<void> DisplayModes::getDefaultDisplayMode(getDefaultDisplayMode_cb resultCb) {
    DisplayMode mode { mDefaultModeId, kModeMap.at(mDefaultModeId).name };
    resultCb(mode);
    return Void();
}
