...
int main() {
    ...
    err = ioctl(uinputFd, UI_SET_EVBIT, EV_KEY) |
          ioctl(uinputFd, UI_SET_KEYBIT, KEY_MODE_NONE) |
    ...
    uevent_listener.Poll([&uinputFd](const Uevent& uevent) {
        ...
        bool none = uevent.state.find("(null)=0") != std::string::npos;
        bool vibration = uevent.state.find("USB_HOST=0") != std::string::npos;
        bool silent = uevent.state.find("USB=0") != std::string::npos;

        int keyCode;
        if (none && !vibration && !silent) {
            keyCode = KEY_MODE_NONE;
